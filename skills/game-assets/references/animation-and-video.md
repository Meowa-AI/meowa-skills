...
5. **Choose Pixel or HD explicitly.** `--animation-model pixel-engine-v1.1` selects Pixel and `--animation-model frame-engine-v1.1` selects HD. This setting, not the source dimensions, controls the render mode. Pixel applies pixel-specific motion and edge handling and accepts PNG or static WebP. HD supports larger pixel artwork, chibi characters, and small HD or 2D characters, does not apply pixel-specific optimization, and accepts PNG, JPEG, or static WebP. Static WebP is converted to same-size PNG before prompt enhancement or animation. The source must satisfy the selected mode's size limits.
...
Do not run this command until the input has passed the preparation sequence above. Select Pixel or HD explicitly; source dimensions validate that choice but never change it. Static WebP is converted to same-size PNG before prompt enhancement or animation.
...
The public image-type choices are `pixel-engine-v1.1` (Pixel, default) and `frame-engine-v1.1` (HD). `--intelligence` defaults to `standard`; `advanced` uses the newer animation path, costs 40 credits before background removal, and supports every integer frame count from 3 through 16. Standard costs 30 credits before background removal and supports 4, 6, 8, 10, 12, or 16 frames. Background removal adds the shared 0/5/10-credit tier charge. Prompt optimization is enabled by default and uses the same image type and intelligence selection as generation; use `--no-optimize-prompt` only when the supplied prompt is already final.

For pixel animation, `--color-count` accepts 2–64. The four padding options accept independent non-negative pixel counts and add transparent canvas without resizing the source. The runner validates the resulting pixel canvas before submission, preventing a paid request when either axis would exceed 256px for Standard or 320px for Advanced. A 64px character can technically use a canvas above 128px, but 128×128 or smaller is recommended for more consistent animation.
...
The defaults mirror the web UI: 16 frames (2 seconds), Pixel Style, 480P, Detailed quality for Pixel and Standard quality for HD, standard background removal, a `#c6c6c6` solid background, loop motion, and prompt optimization enabled. Frame choices are 8, 16, 24, and 32. For a physically consistent character turnaround without mirrored left/right views, use the Multi-view prompt and 32-frame loop documented under Eight-direction characters in `pixel-and-hd-assets.md`. Pixel inputs must not exceed 320 pixels on the longest side and use 480P; HD inputs do not use that dimension limit and support 480P, 720P, or 1080P. HD 720P costs 10 extra credits. HD 1080P generation costs 30 credits for Standard or 35 for Detailed at every duration; removal charges remain unchanged. At 1080P an omitted `--remove-bg-method` defaults to `none` with a green `#00b140` background (clean it up later for free in the Meowa Background Removal Workshop); 480P/720P keep `standard` + `#c6c6c6`. Pass `--remove-bg-method standard` explicitly to pay for removal at 1080P. `--high-frame-rate` is off by default. When it is set and `--remove-bg-method` is omitted, removal becomes `none` and an omitted `--background-color` becomes green `#00b140`. An explicit removal choice still wins. The flag keeps every generated frame in the output WebP instead of sampling down to 8/16/24/32; pixel mode still pixelates every frame and removal still applies per frame. Pixel defaults to `--alpha-mode sharp`; HD keeps its existing `--alpha-mode soft` default. Both styles accept `soft` to preserve translucent regions and `sharp` to binarize alpha. Background removal choices are `none` and `standard`; `advanced` remains visible but is temporarily unavailable. Standard and Detailed output quality are available; Ultimate remains visible in the product but is still in development.
...
With standard background removal, `--remove-bg-batch-size` accepts `2`, `4`, `8`, or `16` (default): highest, high, medium, or low quality. Removal costs `ceil(output_frames / batch_size) × 5` credits. At 16 frames (2 seconds), batches of 4 cost 20 removal credits, added to generation credits. `none` costs zero removal credits and makes no removal calls. With `none`, the background defaults to green (`#00b140`) for later cleanup in the Meowa Background Removal Workshop; an explicit `--background-color` overrides this.


Add `--last-image-file <last-frame.png>` for first-to-last-frame control. The two images must have identical dimensions. When a last frame is supplied, the runner ignores `--animation-mode loop` and submits `non_loop`, because the exact last frame is the endpoint, and an omitted `--output-frames` defaults to 24 (3 seconds) instead of 16, matching the web UI: 2-second first/last clips tend to freeze between the two frames. Pass `--output-frames` explicitly to keep another duration.
...
Video-reference modes: `--style-mode pixel|hd` (default pixel), `--resolution 480p|720p` (default 480p; 720p only HD), `--remove-bg-method none|standard` (default standard), `--remove-bg-batch-size 2|4|8|16` (default 16), `--high-frame-rate` (default off; omitted removal becomes `none` and an omitted fill becomes `#00b140`; every generated frame is kept in the output WebP without sampling), `--alpha-mode sharp|soft` (default sharp; choose soft for HD translucent edges). Both references must have identical original dimensions. Pixel references must be at most 320 pixels per side. Stage1 keeps original resolution; generation uses nearest-neighbor integer scaling for Pixel or Lanczos scaling for HD, with the shared image-animation padding geometry. Generated Pixel frames use the same perfect-pixel and background-removal ordering as image animation. HD uses the same frame extraction and removal path.
