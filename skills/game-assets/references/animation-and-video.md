...
5. **Choose Pixel or HD explicitly.** `--animation-model pixel-engine-v1.1` selects Pixel and `--animation-model frame-engine-v1.1` selects HD. This setting, not the source dimensions, controls the render mode. Pixel applies pixel-specific motion and edge handling. HD supports larger pixel artwork, chibi characters, and small HD or 2D characters, but it does not apply pixel-specific optimization. The source must satisfy the selected mode's size and format limits.
...
Do not run this command until the input has passed the preparation sequence above. Select Pixel or HD explicitly; source dimensions validate that choice but never change it.
