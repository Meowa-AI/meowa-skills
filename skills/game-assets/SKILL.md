...
4. **Animation:** For reference-video editing, Pixel/HD, resolution, alpha and background-removal options mirror the web card (see Animation and video); `--background-color` (default `#ffffff`) fills transparent pixels in both reference inputs; use `meowa-animation-edit-prompts` to prepare editable text and `meowa-animation-edit-run` to generate after filling video content and, if supplied, image content; polishing is manual. See [Animation and video](references/animation-and-video.md). finalize the still asset before animation, then follow the prompt, mode-routing, and motion-canvas checks in [Animation and video](references/animation-and-video.md). Use `meowa-animation-run` for the new 8/16/24/32-frame Pixel or HD path (HD supports 480P/720P/1080P; 1080P generation is 30/35 credits for Standard/Detailed, plus the existing removal fee), or `animate-run` for the established frame-animation path. For an ordinary but complex action, prepare intermediate poses and use `keyframes-run` before considering video. Use `video-run` only as a fallback for motion that frame animation cannot represent or when higher video resolution is required.
